...
def process_and_save(input_path: str, output_path: str, threshold: int = 80):
    ...
    for _, row in df.iterrows():
        ...
        out = {
            ...
            "owner_score": score,
            "evidence_phrases": (
                "|".join(ty.get("evidence_phrases", []))
                if isinstance(ty.get("evidence_phrases", []), list)
                else ""
            ),
            "risk_flags": (
                "|".join(ty.get("risk_flags", []))
                if isinstance(ty.get("risk_flags", []), list)
                else ""
            ),
            "extracted_property_type": (ty.get("extracted") or {}).get(
                "property_type", None
            ),
            ...
            "extracted_location_text": (ty.get("extracted") or {}).get(
                "location_text", None
            ),
        }
        if score >= threshold:
